...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    H = hidden_dim
    C = num_classes
    self.params['W1'] = np.random.normal(scale=weight_scale, size=(D,H))
    self.params['b1'] = np.zeros((H,))
    self.params['W2'] = np.random.normal(scale=weight_scale, size=(H,C))
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    hidden, l1_cache = affine_relu_forward(X, self.params['W1'], self.params['b1'])
    scores, l2_cache = affine_forward(hidden, self.params['W2'], self.params['b2'])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dhidden, dW2, db2 = affine_backward(dscores, l2_cache)
    dX, dW1, db1 = affine_relu_backward(dhidden, l1_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i, (d_in, d_out) in enumerate(zip(dims, dims[1:])):
      w = np.random.normal(scale=weight_scale, size=(d_in, d_out))
      b = np.zeros((d_out,), dtype=dtype)
  ...
  def loss(self, X, y=None):
    ...
    w = self.params['W%d' % n]
    b = self.params['b%d' % n]
    scores,cache = affine_forward(input, w, b)
    caches[n] = cache
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    loss, dscores = softmax_loss(scores, y)
    n = self.num_layers
    grad, dW, db = affine_backward(dscores, caches[n])
    grads['W%d' % n] = dW
    grads['b%d' % n] = db
